	...
	public function index()
	{
		if ($this->role->i('read'))
		{
			if ( $this->input->is_ajax_request() ) 
			{
				...
				elseif ($this->input->post('act') == 'environment')
				{
					$environment = $this->input->post('value');
					$path = FCPATH.'environment.php'; 
					$content = "<?php define('ENVIRONMENT', isset(\$_SERVER['CI_ENV']) ? \$_SERVER['CI_ENV'] : '{$environment}');";
					
					fopen($path, "r") or die("Could not open file!");
					write_file($path, $content);

					$response['alert']['type'] = 'info';
					$response['alert']['content'] = "ENVIRONMENT set to <b class='text-danger'>$environment</b>";
					$this->json_output($response);
				}
